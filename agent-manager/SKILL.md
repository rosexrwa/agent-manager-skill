...
The `grok` provider (aliases: `grok-cli`, `grok-build`) resolves the `grok` executable from `$HOME/.local/bin`, `$HOME/.grok/bin`, common user-local paths, and system paths. Model and execution options belong in `launcher_args` (`-m`/`--model`, `--always-approve`/`--yolo`). System prompt injection uses `--append-system-prompt`. Session restore uses `--resume <id>` when a stored session id exists. MCP servers are managed with `grok mcp`, not a launch-time JSON flag. Authentication is inherited from the managed shell (`grok login`, or a shell env such as `XAI_API_KEY`); credentials must never appear in `launcher_args`, agent YAML, pane captures, logs, or error messages. If the CLI is missing, `start` fails with an install/`grok login` hint and does not create a tmux session.
